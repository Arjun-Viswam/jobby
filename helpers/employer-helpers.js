...
const { ConferenceList } = require('twilio/lib/rest/api/v2010/account/conference')
...
module.exports = {
    ...
    doEmpSignup: (userData) => {
        return new Promise(async (resolve, reject) => {
            if(userData.password == userData.password2){
                ...
                db.get().collection(collection.EMPLOYER_COLLECTION).insertOne(userDetails).then((data) => {
    ...
    getEmpDetails:(mobile) => {
        return new Promise(async (resolve, reject) => {
            let employer = await db.get().collection(collection.EMPLOYER_COLLECTION).findOne({ mobile: mobile })
            resolve(employer)
        })
    },
    getEmployerDetails:(empId) => {
        return new Promise(async (resolve, reject) => {
            let employers = await db.get().collection(collection.EMPLOYER_COLLECTION).findOne({ _id : objectId(empId)})
            resolve(employers)
        })
    },
    ...
    getSkills : (empId)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.EMPLOYER_COLLECTION).findOne({_id : objectId(empId)}).then((data)=>{
                resolve(data.skills)
            })
        })
    },
    ...
    addProfileImage:(empId)=>{
     ...
     getJobs : (employerId)=>{
         return new Promise(async(resolve,reject)=>{
            let jobs = await db.get().collection(collection.JOB_COLLECTION).aggregate({ $match : {empId : objectId(employerId)}},
                {
                    $group:{_id : null,
                        empId : "$employerId"
                     }  
     ...
     getJobDetails : (jobId)=>{
         return new Promise(async(resolve,reject)=>{
             db.get().collection(collection.JOB_COLLECTION).findOne({_id : objectId(jobId)}).then((jobDetails)=>{
                 resolve(jobDetails)
             })
         })
     },
     ...
     getAppliedCandidates : (jobsId)=>{
         return new Promise(async(resolve,reject)=>{
            let candidates = await db.get().collection(collection.JOBS_APPLIED).aggregate({ $match : {jobId : objectId(jobsId)}},
                {
                    $group:{_id : null,
                        jobId : "$jobsId"
                     }
     ...
     sentNotification : (data)=>{
        ...
        today = yyyy + '-' + mm + '-' + dd;
         return new Promise((resolve,reject)=>{
             ...
             details.date = today
             db.get().collection(collection.NOTIFICATION).insertOne(details).then((response)=>{
                 resolve(response)
             })
         })
     },
     ...
     verifyPayment:(details,data,empId,planDetails)=>{
        return new Promise((resolve,reject)=>{
            ...
            if(hmac==details['payment[razorpay_signature]']){
             ...
             db.get().collection(collection.JOB_COLLECTION).insertOne(jobDetails).then(()=>{
    ...
    getjobcount : (empId)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.JOB_COLLECTION).aggregate({$match: {empId : objectId(empId)}},
            {
                $group:{_id : null,
                    jobs : "$jobs"}
            }).toArray().then((jobs)=>{
    ...
    SaveTheNotes : (note,empId)=>{
        return new Promise((resolve,reject)=>{
            ...
            db.get().collection(collection.NOTES_COLLECTION).insertOne(Notes).then(()=>{
                resolve()
            })
        })
    },
    getNotes : (empId)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.NOTES_COLLECTION).aggregate({$match : {empId : objectId(empId)}},
            {
                $group : { _id : null,
                notes : "$notes"}
            }).toArray().then((notes)=>{
    ...
    deleteNotes : (noteId)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.NOTES_COLLECTION).removeOne({_id : objectId(noteId)}).then(()=>{
                resolve()
            })
        })
    },
    ...
    postMachineTest : (data,userId,emp,jobId)=>{
        ...
        return new Promise((resolve,reject)=>{
            ...
            module.status = true
            db.get().collection(collection.MACHINE_TEST).insertOne(module).then(()=>{
                resolve()
            })
        })
    },
    getAnswerKeys : (userId,jobId)=>{
        ...
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.ANSWER_KEYS).aggregate({ $match :{ userId : objectId(userId)}},
            {
