...
module.exports = {
  ...
  getNumberOfJobs: (companyId) => {
    return new Promise(async (resolve, reject) => {
      let jobs = await db
        .get()
        .collection(collection.JOB_COLLECTION)
        .aggregate({ $match: { empId: companyId } })
        .toArray();
      resolve(jobs.length);
    });
  },
  getCompanyDetails: (companyId) => {
    return new Promise((resolve, reject) => {
      db.get()
        .collection(collection.EMPLOYER_COLLECTION)
        .findOne({ _id: objectId(companyId) })
        .then((company) => {
          resolve(company);
        });
    });
  ...
  getTheJobs: (companyId) => {
    ...
    return new Promise(async (resolve, reject) => {
      let jobs = await db
        ...
        .collection(collection.JOB_COLLECTION)
        .aggregate(
          { $match: { empId: objectId(companyId) } },
          {
            $match: { status: "active" },
          },
          {
  ...
  saveJob: (jobId, userId) => {
    return new Promise(async (resolve, reject) => {
      let saved = await db
        .get()
        .collection(collection.SAVED_COLLECTION)
        .findOne({ saved: jobId + userId });
      if (saved) {
        resolve();
      } else {
        let job = await db
  ...
  getSavedJobs: (usersId) => {
    return new Promise(async (resolve, reject) => {
      let savedjob = await db
        .get()
        .collection(collection.SAVED_COLLECTION)
        .aggregate(
          { $match: { userId: objectId(usersId) } },
          {
  ...
  saveCompany: (empId, userId) => {
    return new Promise(async (resolve, reject) => {
      let saved = await db
        .get()
        .collection(collection.SAVE_COMPANY)
        .findOne({ saved: empId + userId });
      if (saved) {
        resolve();
      } else {
  ...
  userSearch: (key1, key2, key3) => {
    return new Promise(async (resolve, reject) => {
      db.get()
        .collection(collection.JOB_COLLECTION)
        .createIndex(
          { jobtype: "text", availability: "text", location: "text" },
