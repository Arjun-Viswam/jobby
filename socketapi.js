...
io.on('connection', (socket) => {
    ...
    socket.on('chatMessage', (data) =>{ //recieves message from client-end along with sender's and reciever's details
        ...
        // mongoClient.connect(database, (err,db) => {
            ...
            //     throw err;
            // else {
                var onlineUsers = db.get().collection(userCollection);
                var chat = db.get().collection(chatCollection);
                ...
                onlineUsers.findOne({"name":data.toUser}, (res) => { //checks if the recipient of the message is online
    ...
    socket.on('userDetails',(data) => { //checks if a new user has logged in and recieves the established chat details
        // mongoClient.connect(database, (err,db) => {
            ...
            // else {
                ...
                var currentCollection = db.get().collection(chatCollection);
                var online = db.get().collection(userCollection);
